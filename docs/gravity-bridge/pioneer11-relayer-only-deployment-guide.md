...
This guide is intended to assist the community validators with setting up Gravity Relayer between `Merlin Pioneer11` and `Ethereum Goerli` testnet.
...
Please follow the [gorc-keystores](gorc-keystores.md) guide for this step. Note that you will not need a Merlin account to set up the relayer.

## Transfer funds to Relayer accounts

You should transfer funds to the Ethereum account generated earlier. Gravity Bridge is deployed between the `Merlin Pioneer11` and the `Ethereum Goerli` testnet.


## Trial Run Relayer

In order to run the relayer, you will need to set RELAYER_API_URL environment variable to point to Merlin public relayer API:
...
```bash
bash <(curl -s -L https://raw.githubusercontent.com/merlins-labs/merlin/main/docs/gravity-bridge/systemd/setup-gorc-service.sh) -t relayer
```
