...
```shell
git clone https://github.com/merlins-labs/merlin.git
```
...
Some test cases will run on both `geth` and `merlin`, you can also select the platform to run using `-k`:

```shell
$ cd integration_tests
$ # run against merlin only
$ pytest -k merlin
