...
Right now Merlin module supports native token to CRC20 token conversion only. As more dApp projects deploy their tokens on Merlin EVM, there is a need to support converting EVM tokens deployed on Merlin EVM to native token for use cases such as bridging to Cosmos Chains by IBC or to Ethereum by Gravity Bridge.
...
EVM Tokens are tokens that adopt the widely used CRC20/CRC21 token standard. These token are smart contracts deployed on Merlin EVM.

### Local EVM Token

Local EVM Tokens are EVM tokens that are originated from Merlin EVM, that means the minting of the tokens happen on Merlin.

### Native Token

Native Tokens are tokens on the Cosmos layer of Merlin. They are not directly usable on Merlin EVM but an intermediate token format between bridge protocols and Merlin EVM because most bridge protocols are Cosmos module that interacts with native tokens.
...
Currently, the native token denom to contract address is registered in the Merlin module kvstore and stored as key-value pairs: 
```
key -> value
...
For example, IBC-ATOM to contract on Merlin mainnet mapping is
```
...
- `merlin0x`: Merlin CRC20/CRC21 Native Token
...
In this ADR, we do not intend to change the mapping registration process. Following the current design, the address specified in the network parameter `MerlinAdmin` has the rights to update the mappings.
...
2. If the token source is Merlin EVM, invoke `keeper.BurnVouchersToEvmCoins()`; Otherwise, invoke `keeper.ConvertVouchersToEvmCoins()`^2;
...
The denom naming of the native token with Merlin EVM as source is
```
merlin0x{smartContractAddress}
```

The chain must avoid naming any native asset with the prefix `merlin0x` in the future.
...
3. Invoke the corresponding smart contract `mint_by_merlin_module` method to mint the EVM token to `receiver` EVM address.
