...
// Proposal
event __MerlinSendToIbc(address indexed sender, uint256 indexed channelId, string recipient, uint256 amount, bytes extraData);
```

#### Description

- Event `__MerlinSendToIbc` has differnt signature in CRC20 and CR21.
...
- Channel ID argument is added to support sending to specified IBC channel.
  - For token originated from Merlin, this channel ID could be any available IBC channel.
...
### __MerlinSendToEVMChain Event

#### Changes

- This is an replacement to the `__MerlinSendToChain` in the current draft of CRC21 standard
...
```solidity
// Current
event __MerlinSendToChain(address sender, address recipient, uint256 amount, uint256 bridge_fee, uint256 chain_id);
...
- `__MerlinSendToAccount`
- `__MerlinSendToChainResponse`
- `__MerlinCancelSendToChain`
- `__MerlinSendCroToIbc`
...
* https://github.com/merlins-labs/merlin/issues/546
* https://github.com/merlins-labs/merlin/pull/545
