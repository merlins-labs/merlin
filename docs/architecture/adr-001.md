...
2. "cmd/merlind/cmd/root.go" which adds Gravity Bridge-related commands.

In addition to that, the "x/merlin" module contains Gravity Bridge-related code in its types and keeper. This following code will be removed:
1. the whole x/merlin/keeper/gravity_hooks.go module
2. the gravityKeeper field in MerlinKeeper
...
Once we decide that the Gravity Bridge code is mature enough to be deployed in mainnet, the x/merlin module `ConsensusVersion` should be increased and the corresponding upgrade handler should be added. At that time, we could also remove the flag.

** Flag removal

The unsafe-experimental flag will be removed from the code base as the module is stable enough to be added to merlin. Moreover, the logic of the gravity module can be controlled through the governance parameter "bridge_active" which still leave us the option to disable it if necessary.
...
### Positive
* Reduced complexity in the initial Merlin state
