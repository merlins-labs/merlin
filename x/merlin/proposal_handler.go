...
func NewTokenMappingChangeProposalHandler(k keeper.Keeper) govtypes.Handler {
	return func(ctx sdk.Context, content govtypes.Content) error {
		...
		case *types.TokenMappingChangeProposal:
			// check first that the denom is one of the denom supported by merlin
		...
		default:
			return errors.Wrapf(sdkerrors.ErrUnknownRequest, "unrecognized merlin proposal content type: %T", c)
		}
	}
}
