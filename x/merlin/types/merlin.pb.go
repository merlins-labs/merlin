...
type Params struct {
	IbcCroDenom string `protobuf:"bytes,1,opt,name=ibc_mer_denom,json=ibcCroDenom,proto3" json:"ibc_mer_denom,omitempty" yaml:"ibc_mer_denom,omitempty"`
	...
	MerlinAdmin          string `protobuf:"bytes,3,opt,name=merlin_admin,json=merlinAdmin,proto3" json:"merlin_admin,omitempty"`
...
func (m *Params) GetMerlinAdmin() string {
	if m != nil {
		return m.MerlinAdmin
	}
	return ""
}
...
func init() {
	proto.RegisterType((*Params)(nil), "merlin.Params")
	proto.RegisterType((*TokenMappingChangeProposal)(nil), "merlin.TokenMappingChangeProposal")
	proto.RegisterType((*TokenMapping)(nil), "merlin.TokenMapping")
}

func init() { proto.RegisterFile("merlin/merlin.proto", fileDescriptor_8bc54992a93db2d2) }
...
func (m *Params) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.MerlinAdmin) > 0 {
		i -= len(m.MerlinAdmin)
		copy(dAtA[i:], m.MerlinAdmin)
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.MerlinAdmin)))
		i--
		dAtA[i] = 0x1a
	}
	if m.IbcTimeout != 0 {
		i = encodeVarintMerlin(dAtA, i, uint64(m.IbcTimeout))
		i--
		dAtA[i] = 0x10
	}
	if len(m.IbcCroDenom) > 0 {
		...
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.IbcCroDenom)))
		i--
		dAtA[i] = 0xa
	}
...
func (m *TokenMappingChangeProposal) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	_ = l
	if m.Decimal != 0 {
		i = encodeVarintMerlin(dAtA, i, uint64(m.Decimal))
		i--
		dAtA[i] = 0x30
	}
	if len(m.Symbol) > 0 {
		...
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.Symbol)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.Contract) > 0 {
		...
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.Contract)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.Denom) > 0 {
		i -= len(m.Denom)
		copy(dAtA[i:], m.Denom)
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.Denom)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Description) > 0 {
		...
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.Description)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.Title) > 0 {
		i -= len(m.Title)
		copy(dAtA[i:], m.Title)
		i = encodeVarintMerlin(dAtA, i, uint64(len(m.Title)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func encodeVarintMerlin(dAtA []byte, offset int, v uint64) int {
	offset -= sovMerlin(v)
	base := offset
	for v >= 1<<7 {
...
func (m *Params) Size() (n int) {
	...
	l = len(m.IbcCroDenom)
	if l > 0 {
		n += 1 + l + sovMerlin(uint64(l))
	}
	if m.IbcTimeout != 0 {
		n += 1 + sovMerlin(uint64(m.IbcTimeout))
	}
	l = len(m.MerlinAdmin)
	if l > 0 {
		n += 1 + l + sovMerlin(uint64(l))
...
func (m *TokenMappingChangeProposal) Size() (n int) {
	...
	if m.Decimal != 0 {
		n += 1 + sovMerlin(uint64(m.Decimal))
	}
	return n
}
...
func sovMerlin(x uint64) (n int) {
	return (math_bits.Len64(x|1) + 6) / 7
}
func sozMerlin(x uint64) (n int) {
	return sovMerlin(uint64((x << 1) ^ uint64((int64(x) >> 63))))
}
func (m *Params) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field MerlinAdmin", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.MerlinAdmin = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		case 4:
			if wireType != 0 {
		...
		default:
			iNdEx = preIndex
			skippy, err := skipMerlin(dAtA[iNdEx:])
			if err != nil {
				return err
			}
...
func skipMerlin(dAtA []byte) (n int, err error) {
	l := len(dAtA)
	iNdEx := 0
	depth := 0
	for iNdEx < l {
		...
		case 2:
			...
			if length < 0 {
				return 0, ErrInvalidLengthMerlin
			}
			iNdEx += length
		case 3:
			depth++
		case 4:
			if depth == 0 {
				return 0, ErrUnexpectedEndOfGroupMerlin
			}
			depth--
		case 5:
			iNdEx += 4
...
var (
	ErrInvalidLengthMerlin        = fmt.Errorf("proto: negative length found during unmarshaling")
	ErrIntOverflowMerlin          = fmt.Errorf("proto: integer overflow")
	ErrUnexpectedEndOfGroupMerlin = fmt.Errorf("proto: unexpected end of group")
)
