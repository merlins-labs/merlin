...
const (
	// MerlinNamespace is the extension RPC namespace of merlin module.
	MerlinNamespace = "merlin"
...
func init() {
	if err := evmrpc.RegisterAPINamespace(MerlinNamespace, CreateMerlinRPCAPIs); err != nil {
		panic(err)
	}
}

// CreateMerlinRPCAPIs creates extension json-rpc apis
func CreateMerlinRPCAPIs(ctx *server.Context, clientCtx client.Context, tmWSClient *rpcclient.WSClient, allowUnprotectedTxs bool, indexer ethermint.EVMTxIndexer) []rpc.API {
	evmBackend := backend.NewBackend(ctx, ctx.Logger, clientCtx, allowUnprotectedTxs, indexer)
	return []rpc.API{
		{
			Namespace: MerlinNamespace,
			Version:   apiVersion,
			Service:   NewMerlinAPI(ctx.Logger, clientCtx, *evmBackend),
			Public:    true,
		},
	}
}

// MerlinAPI is the extension jsonrpc apis prefixed with merlin_.
type MerlinAPI struct {
	...
	merlinQueryClient types.QueryClient
}

// NewMerlinAPI creates an instance of the merlin web3 extension apis.
func NewMerlinAPI(
	logger log.Logger,
	clientCtx client.Context,
	backend backend.Backend,
	...
	return &MerlinAPI{
		...
		backend:           backend,
		merlinQueryClient: types.NewQueryClient(clientCtx),
	}
}

func (api *MerlinAPI) getBlockDetail(blockNrOrHash rpctypes.BlockNumberOrHash) (
...
func (api *MerlinAPI) GetTransactionReceiptsByBlock(blockNrOrHash rpctypes.BlockNumberOrHash) ([]map[string]interface{}, error) {
	api.logger.Debug("merlin_getTransactionReceiptsByBlock", "blockNrOrHash", blockNrOrHash)
...
func (api *MerlinAPI) ReplayBlock(blockNrOrHash rpctypes.BlockNumberOrHash, postUpgrade bool) ([]map[string]interface{}, error) {
	api.logger.Debug("merlin_replayBlock", "blockNrOrHash", blockNrOrHash)
	resBlock, blockNumber, blockHash, blockRes, baseFee, err := api.getBlockDetail(blockNrOrHash)
	if err != nil {
		return nil, err
	}
	...
	rsp, err := api.merlinQueryClient.ReplayBlock(rpctypes.ContextWithHeight(contextHeight), req)
	if err != nil {
		return nil, err
	}
...
func (api *MerlinAPI) getBlock(blockNrOrHash rpctypes.BlockNumberOrHash) (blk *coretypes.ResultBlock, err error) {
