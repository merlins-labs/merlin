...
func (suite *KeeperTestSuite) TestConvertVouchersToEvmCoins() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				contract, found := suite.app.MerlinKeeper.GetContractByDenom(suite.ctx, CorrectIbcDenom)
				suite.Require().True(found)
				ret, err := suite.app.MerlinKeeper.CallModuleCRC21(suite.ctx, contract, "balanceOf", common.BytesToAddress(address.Bytes()))
				suite.Require().NoError(err)
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			err := suite.app.MerlinKeeper.ConvertVouchersToEvmCoins(suite.ctx, tc.from, tc.coin)
...
func (suite *KeeperTestSuite) TestIbcTransferCoins() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				suite.app.MerlinKeeper.SetAutoContractForDenom(suite.ctx, "incorrect", common.HexToAddress("0x11"))
			},
			errors.New("the coin incorrect is neither an ibc voucher or a merlin token"),
			func() {
			},
		},
		{
			...
			func() {
				...
				suite.MintCoins(address, sdk.NewCoins(sdk.NewCoin(CorrectIbcDenom, sdk.NewInt(123))))
				// Add support for the IBC token
				suite.app.MerlinKeeper.SetAutoContractForDenom(suite.ctx, CorrectIbcDenom, common.HexToAddress("0x11"))
			},
			nil,
			func() {
			},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			err := suite.app.MerlinKeeper.IbcTransferCoins(suite.ctx, tc.from, tc.to, tc.coin)
			if tc.expectedError != nil {
				suite.Require().EqualError(err, tc.expectedError.Error())
			} else {
				suite.Require().NoError(err)
