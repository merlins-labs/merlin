...
func (k Keeper) ConvertVouchersToEvmCoins(ctx sdk.Context, from string, coins sdk.Coins) error {
	...
	for _, c := range coins {
		...
		case params.IbcCroDenom:
			...
			// Compute new amount, because basemer is a 8 decimals token, we need to multiply by 10^10 to make it
			// a 18 decimals token
...
func (k Keeper) IbcTransferCoins(ctx sdk.Context, from, destination string, coins sdk.Coins) error {
	...
	for _, c := range coins {
		...
		default:
			if !types.IsValidIBCDenom(c.Denom) && !types.IsValidMerlinDenom(c.Denom) {
				return fmt.Errorf("the coin %s is neither an ibc voucher or a merlin token", c.Denom)
			}
...
func (k Keeper) ibcSendTransfer(ctx sdk.Context, sender sdk.AccAddress, destination string, coin sdk.Coin) error {
	...
	if types.IsSourceCoin(coin.Denom) {
		...
		// we are using the mer denom to extract the channel id
