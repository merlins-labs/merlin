...
func (k Keeper) doAfterSendToCosmosEvent(ctx sdk.Context, event gravitytypes.SendToCosmosEvent) error {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		...
		sdk.NewAttribute(merlintypes.AttributeKeySender, event.GetEthereumSender()),
		sdk.NewAttribute(merlintypes.AttributeKeyReceiver, event.GetCosmosReceiver()),
		sdk.NewAttribute(merlintypes.AttributeKeyAmount, coins.String()),
		sdk.NewAttribute(gravitytypes.AttributeKeyBridgeChainID, strconv.FormatUint(
			k.gravityKeeper.GetParams(ctx).BridgeChainId, 10,
		)),
		sdk.NewAttribute(merlintypes.AttributeKeyEthereumTokenContract, event.GetTokenContract()),
