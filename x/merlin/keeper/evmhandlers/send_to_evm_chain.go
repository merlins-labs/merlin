...
const (
	SendToEvmChainEventName         = "__MerlinSendToEvmChain"
	SendToEvmChainResponseEventName = "__MerlinSendToEvmChainResponse"
)
...
var (
	...
	// `event __MerlinSendToChainResponse(uint256 id)`
...
// SendToEvmChainHandler handles `__MerlinSendToEvmChain` log
...
func NewSendToEvmChainHandler(gravitySrv gravitytypes.MsgServer, bankKeeper types.BankKeeper, merlinKeeper merlinkeeper.Keeper) *SendToEvmChainHandler {
...
// Handle `__MerlinSendToChain` log only if gravity is activated.
func (h SendToEvmChainHandler) Handle(
	...
	if len(topics) != 4 {
		// log and ignore
		h.merlinKeeper.Logger(ctx).Info("log signature matches but wrong number of indexed events")
		for i, topic := range topics {
			h.merlinKeeper.Logger(ctx).Debug(fmt.Sprintf("topic index: %d value: %s", i, topic.TerminalString()))
		}
		return nil
	}
	...
	if !types.IsValidGravityDenom(denom) && !types.IsValidMerlinDenom(denom) {
		return fmt.Errorf("the native token associated with the contract %s is neither a gravity voucher or a merlin token", contract)
	}
