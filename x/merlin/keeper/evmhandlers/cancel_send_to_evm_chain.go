...
const CancelSendToEvmChainEventName = "__MerlinCancelSendToEvmChain"

// CancelSendToEvmChainEvent represent the signature of
// `event __MerlinCancelSendToEvmChain(uint256 id)`
...
// CancelSendToEvmChainHandler handles `__MerlinCancelSendToEvmChain` log
...
// Handle `__MerlinCancelSendToChain` log only if gravity is activated.
func (h CancelSendToEvmChainHandler) Handle(
	...
	crc20Address, found := h.merlinKeeper.GetContractByDenom(ctx, denom)
	if !found {
		return fmt.Errorf("the native token %s is not associated with any contract address on merlin", denom)
	}
	...
	err = h.merlinKeeper.ConvertVouchersToEvmCoins(ctx, senderCosmosAddr.String(), refundAmount)
	if err != nil {
		return err
	}
	return nil
