...
// `event __MerlinSendToIbc(address indexed sender, string indexed recipient, string indexed channel_id, uint256 amount, bytes extraData)`
...
// SendToIbcV2Handler handles `__MerlinSendToIbc` log
type SendToIbcV2Handler struct {
	*SendToIbcHandler
}

func NewSendToIbcV2Handler(bankKeeper types.BankKeeper, merlinKeeper merlinkeeper.Keeper) *SendToIbcV2Handler {
	return &SendToIbcV2Handler{
		SendToIbcHandler: NewSendToIbcHandler(bankKeeper, merlinKeeper),
	}
}
