...
func (suite *KeeperTestSuite) RegisterSourceToken(
	...
	return suite.app.MerlinKeeper.RegisterOrUpdateTokenMapping(suite.ctx, &msg)
}
...
func (suite *KeeperTestSuite) TestOnRecvVouchers() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			suite.app.MerlinKeeper.OnRecvVouchers(suite.ctx, tc.coins, address.String())
			tc.postCheck()
		})
	}
}
...
func (suite *KeeperTestSuite) TestRegisterOrUpdateTokenMapping() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := suite.app.MerlinKeeper.SetExternalContractForDenom(
					suite.ctx,
		...
		},
		{
			...
			types.MsgUpdateTokenMapping{
				Sender:   "",
				Denom:    "merlin0xA6d4fecb1a6fb7c2ca350169a050d483bd87b883",
				Contract: contractAddress,
				Symbol:   "",
				Decimal:  0,
			},
		...
		},
		{
			...
			types.MsgUpdateTokenMapping{
				Sender:   "",
				Denom:    "merlin0xf6d4fecb1a6fb7c2ca350169a050d483bd87b883",
				Contract: contractAddress,
				Symbol:   "",
				Decimal:  0,
			},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			err := suite.app.MerlinKeeper.RegisterOrUpdateTokenMapping(suite.ctx, &tc.msg)
			if tc.error {
				suite.Require().Error(err)
			} else {
				suite.Require().NoError(err)
