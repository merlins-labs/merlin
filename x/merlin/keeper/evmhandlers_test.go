...
func (suite *KeeperTestSuite) TestSendToAccountHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.MerlinKeeper.SetExternalContractForDenom(suite.ctx, denom, contract)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			handler := evmhandlers.NewSendToAccountHandler(suite.app.BankKeeper, suite.app.MerlinKeeper)
			tc.malleate()
...
func (suite *KeeperTestSuite) TestSendToEvmChainHandler() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				suite.app.MerlinKeeper.SetExternalContractForDenom(suite.ctx, invalidDenom, contract)
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither a gravity voucher or a merlin token"),
		},
		{
			"non supported network id",
			func() {
				suite.app.MerlinKeeper.SetExternalContractForDenom(suite.ctx, validDenom, contract)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToEvmChainHandler(
				...
				suite.app.BankKeeper, suite.app.MerlinKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither an ibc voucher or a merlin token"),
		},
		{
			"success send to ibc",
			func() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcHandler(suite.app.BankKeeper, merlinKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcV2Handler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcV2Handler(suite.app.BankKeeper, merlinKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendCroToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				...
				merCoin := sdk.NewCoin(suite.evmParam.EvmDenom, sdk.NewInt(500))
				merBalance := suite.app.BankKeeper.GetBalance(suite.ctx, sdk.AccAddress(sender.Bytes()), suite.evmParam.EvmDenom)
				suite.Require().Equal(merCoin, merBalance)
			},
			nil,
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendCroToIbcHandler(suite.app.BankKeeper, merlinKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewCancelSendToEvmChainHandler(
				gravitykeeper.NewMsgServerImpl(suite.app.GravityKeeper),
				suite.app.MerlinKeeper, suite.app.GravityKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
