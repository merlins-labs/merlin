...
func SimulateUpdateTokenMapping(ak types.AccountKeeper, bk types.BankKeeper, k *keeper.Keeper) simtypes.Operation {
	return func(
		...
		merlinAdmin := k.GetParams(ctx).MerlinAdmin
		var simAccount simtypes.Account

		if r.Intn(2) > 0 {
			var found bool
			simAccount, found = findMerlinAdmin(accs, merlinAdmin)
			if !found {
		...
		if simAccount.Address.String() != merlinAdmin && errors.Is(err, errorsmod.Wrap(sdkerrors.ErrInvalidAddress, "msg sender is authorized")) {
...
func findMerlinAdmin(accs []simtypes.Account, merlinAdmin string) (simtypes.Account, bool) {
	found := false
	for _, acc := range accs {
		if acc.Address.String() == merlinAdmin {
			found = true
			return acc, found
		}
	}
