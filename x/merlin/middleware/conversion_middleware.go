...
func NewIBCConversionModule(app porttypes.IBCModule, ck merlinkeeper.Keeper) IBCConversionModule {
...
func (im IBCConversionModule) convertVouchers(ctx sdk.Context, data transferTypes.FungibleTokenPacketData, denom string, isSender bool) error {
	...
	if isSender {
		im.merlinkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Sender)
	} else {
		im.merlinkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Receiver)
	}
	return nil
}

func (im IBCConversionModule) canBeConverted(ctx sdk.Context, denom string) bool {
	params := im.merlinkeeper.GetParams(ctx)
	if denom == params.IbcCroDenom {
		return true
	}
	_, found := im.merlinkeeper.GetContractByDenom(ctx, denom)
	return found
}
